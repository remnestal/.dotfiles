...
// ActivateSubscription is an HTTP handler that "activates" the subscription for
// a customer. In terms of Stripe, "activation" means that a subscription
// resource is created from scratch. If there is already at least one non-
// canceled subscription in Stripe for this customer; this is interpreted as
// a situation where the subscription has already been activated. If this is the
// case, this handler returns an error instead of creating another subscription
func ActivateSubscription(w http.ResponseWriter, r *http.Request) {
	...
	for productId, quantity := range quantities {
		...
		// Must create temporary variable for storing a pointer to the current
	...
	// Set customer and API clients as active in the database
	if err := queries.ActivateCustomer(customer.Id); err != nil {
		reqLogger.WithError(err).Errorln("Unable to activate customer in the database")
		httphelper.Fail(w, reqLogger,
			http.StatusInternalServerError,
			errorspec.DatabaseFailure)
		return
...
// CancelSubscription is an HTTP handler that cancels the customer's default
// subscription in Stripe. If there is currenty no active subscription, an error
// is returned instead
func CancelSubscription(w http.ResponseWriter, r *http.Request) {
	...
	// If there is no default Stripe subscription, deactivation fails
	if stripeSub == nil {
		httphelper.Fail(w, reqLogger,
			http.StatusBadRequest,
			errorspec.NoActiveSubscription)
		return
	}
	// Delete the subscription in Stripe
	_, err = stripe.CancelSubscription(stripeSub.ID, nil)
	if err != nil {
		errspec := stripe.ParseError(err)
		if errspec.Critical {
			reqLogger.WithError(err).Errorln("Unable to cancel Stripe subscription")
		}
		httphelper.Fail(w, reqLogger,
			http.StatusInternalServerError,
			errspec.Code)
	...
	// TODO: set some kind of to-be deactivated-flag in DB (sub period-end) so
	// that the API clients can be locked in time?
